...
class AdvTransform:
    '''
    Implemented by ZT, Modified from TrainTransform and ValTransform
    取消了TrainTransform里的数据增强, 保留了TrainTransform里的target
    '''
    def __init__(self, rgb_means=None, std=None, max_labels=1500):
        self.means = rgb_means
        self.std = std
        self.max_labels = max_labels
    ...
    def __call__(self, image, targets, input_dim):
        ...
        image_t, r_ = preproc(image, input_dim, self.means, self.std)
        # boxes [xyxy] 2 [cx,cy,w,h]
        boxes = xyxy2cxcywh(boxes)
        boxes *= r_
        ...
        padded_labels = np.ascontiguousarray(padded_labels, dtype=np.float32)
        image_t = np.ascontiguousarray(image_t, dtype=np.float32)
        #  added by F&F attack, for debug
        # myboxes = padded_labels[:, 1:5].copy()
        # myboxes[:, [0,1]] -= myboxes[:, [2,3]] * 0.5
        # myboxes[:, [2, 3]] += myboxes[:, [0,1]]
        # print(myboxes[:, 2].max(), myboxes[:, 3].max())
        #
        return image_t, padded_labels